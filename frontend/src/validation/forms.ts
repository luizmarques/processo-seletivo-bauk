...
const passwordSchema = z
  .string()
  .min(8, 'A senha deve conter ao menos 8 caracteres, uma letra maiúscula e um número.')
  .regex(/^(?=.*[A-Z])(?=.*\d).+$/, 'A senha deve conter ao menos 8 caracteres, uma letra maiúscula e um número.');
...
export const transferFormSchema = z
  .object({
    ...
    transferAmount: z
      .number()
      .refine((value) => Number.isFinite(value), 'Informe um valor válido para a transferência.')
  ...
  .superRefine((data, context) => {
    if (data.currentUsername && data.recipientUsername === data.currentUsername) {
      context.addIssue({
