...
describe("Value Objects", () => {
  ...
  it("bloqueia usernames iguais quando a regra exige diferenca", () => {
    ...
    expect(() => username.ensureDifferentFrom(sameUsername)).toThrow(
      "Username deve ser diferente.",
    );
  });
  ...
  it("recusa senha sem numero ou sem maiuscula mesmo com tamanho suficiente", () => {
    expect(() => new PlainPassword("Senhasemnumero")).toThrow(
  ...
  it("aceita hash bcrypt valido e recusa hash invalido", () => {
    ...
    ).toBe("$2a$10$7EqJtq98hPqEX7fNZaFWoOhiB0JzZMfjNV8iPBUFeCFGXFq8iDS.e");
    expect(() => new PasswordHash("hash-invalido")).toThrow(
      ValidationDomainError,
    );
  });
  ...
  it("aceita variantes de hash bcrypt suportadas", () => {
    expect(
      new PasswordHash(
        "$2b$10$7EqJtq98hPqEX7fNZaFWoOhiB0JzZMfjNV8iPBUFeCFGXFq8iDS.e",
      ).toString(),
    ).toContain("$2b$10$");
  });
  ...
  it("normaliza chave de idempotencia recebida externamente", () => {
    const key = IdempotencyKey.fromRaw(
