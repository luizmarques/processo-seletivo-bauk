...
async function seed(appDataSource: DataSource): Promise<void> {
  ...
  for (let index = 0; index < 20; index += 1) {
    ...
    const value = new Decimal((index % 5) + 1).mul(5).plus(new Decimal(index).div(10000)).toFixed(4);

    const senderAccount = await accountRepo.findOneByOrFail({ id: sender.accountId });
    const recipientAccount = await accountRepo.findOneByOrFail({ id: recipient.accountId });
    senderAccount.balance = new Decimal(senderAccount.balance).minus(value).toFixed(4);
    recipientAccount.balance = new Decimal(recipientAccount.balance).plus(value).toFixed(4);
