...
describe('TypeOrmUserRepository', () => {
  function createSut(options?: {
    ...
    const accountEntity = { balance: REQUIRED_INITIAL_BALANCE } as AccountEntity;
    const userEntity = {
      username: 'janedoe',
    ...
    const manager = {
      create(entity: typeof AccountEntity | typeof UserEntity, input: Record<string, unknown>) {
    ...
    const entityManager = {
      async transaction<T>(callback: (transactionManager: typeof manager) => Promise<T>): Promise<T> {
        return callback(manager);
      },
    };

    return {
      repository: new TypeOrmUserRepository({} as never, entityManager as never),
      saveCalls,
    };
  }
  ...
  it('propaga erro quando a persistencia da conta falha antes de criar o usuario', async () => {
    ...
    const { repository, saveCalls } = createSut({ saveAccountError: accountError });
