...
export class ListTransactionsUseCase {
  ...
  async execute(input: {
    ...
    const result = await this.transactionRepository.listByAccount({ ...input, accountId: accountId.toString() });
    return {
      data: result.data.map((transaction) => ({
        ...
        value: formatMoneyForDisplay(new TransferAmount(transaction.value).toString()),
        createdAt: transaction.createdAt,
        type: transaction.debitedAccountId === accountId.toString() ? 'cash-out' : 'cash-in',
      })),
