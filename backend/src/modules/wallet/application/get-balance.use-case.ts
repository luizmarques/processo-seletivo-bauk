...
export class GetBalanceUseCase {
  ...
  async execute(accountId: string): Promise<{ balance: string }> {
    ...
    const account = await this.accountRepository.findById(resolvedAccountId.toString());
    if (!account) {
      throw new ResourceNotFoundError('Conta não encontrada.');
    }
    return { balance: formatMoneyForDisplay(new Balance(account.balance).toString()) };
  }
}
