...
class FakePasswordComparator {
  ...
  async compare(value: { toString(): string }, hashed: { toString(): string }): Promise<boolean> {
    this.compareCalls.push({ value: value.toString(), hashed: hashed.toString() });
    return this.shouldMatch;
  }
}

class FakeTokenService {
  public signCalls: Array<{ sub: string; username: string; accountId: string }> = [];

  async sign(payload: { sub: { toString(): string }; username: { toString(): string }; accountId: { toString(): string } }): Promise<string> {
    this.signCalls.push({
...
describe('LoginUseCase', () => {
  it('retorna token com payload correto ao autenticar', async () => {
    ...
    const sut = new LoginUseCase(repository as never, passwordHasher as never, tokenService as never);
    ...
    expect(passwordHasher.compareCalls).toEqual([
      { value: 'Senha123', hashed: '$2a$10$7EqJtq98hPqEX7fNZaFWoOhiB0JzZMfjNV8iPBUFeCFGXFq8iDS.e' },
    ]);
