...
import { PASSWORD_HASHER, TOKEN_SERVICE, USER_REPOSITORY } from '../../../shared/constants/injection-tokens';
...
export class LoginUseCase {
  ...
  async execute(input: { username: string; password: string }): Promise<{ accessToken: string }> {
    ...
    const matches = await this.passwordHasher.compare(password, new PasswordHash(user.password));
    if (!matches) {
      throw new AuthenticationError('Credenciais inválidas.');
    }
